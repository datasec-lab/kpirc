...
CSV_PATH="$RESULTS_DIR/table_vii.csv"
...
main() {
  ...
  for scheme in TCWKPIR TCAKPIR TCAKPIR_OPT; do
    for log_m in 16 18 20; do
      for data_bits in 512 1024 2048; do
        run_case "$scheme" 64 "$log_m" "$data_bits" "$warmup_runs" "$measured_runs"
      done
    done
  done
